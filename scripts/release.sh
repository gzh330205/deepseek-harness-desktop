...
- **适配新版 DSH 认证**：DSH 0.1.2-alpha.2+ 的 \`dsh web\` 引入浏览器一次性认证（匿名访问返回 401）。桌面端以 CLI 输出的带 token 认证地址判定服务就绪，并自动完成认证登录，安装版与开发版均可正常打开 DSH 页面。
- **避免页面二次刷新**：启动页与 DSH 同站（开发模式 127.0.0.1）时认证一次到位，不再执行多余的第二次导航；安装版（跨站）自动采用两步认证保证登录生效。
