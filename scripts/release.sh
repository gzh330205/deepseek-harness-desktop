...
- **修复认证页 401（根本解决）**：DSH 0.1.2-alpha.2+ 的 \`dsh web\` 带浏览器一次性认证（匿名访问返回 401）。桌面端发布版现在把启动页也托管在 127.0.0.1 回环端口，使启动页与 DSH 页面同站——访问带 token 的认证地址后浏览器自动携带登录 cookie 完成跳转，单次加载即打开 DSH 页面，无 401 提示、无二次刷新。
